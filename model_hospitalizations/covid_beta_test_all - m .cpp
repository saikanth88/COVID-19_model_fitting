...
OutputParams covid19(int iter, int t_max, int n_realz, CountyParams par, double *beta_vals, double *mgrt_vals);
...
int main() {
    ...
    while(getline( fin, line_beta)){
        ...
        while (getline(iss, str_beta, ',')){}
            stringstream(str_beta) >> daily_beta[i];
            i++;
    }
    fin.close();
    ...
    double *mgrt_vals = new double[t_max];

    for(int i = 0; i < t_max; i++){
      beta_vals[i] = daily_beta[i];
    }
...
//            cout<<best_beta[col]<<' ';
        ...
        for (int i = 0; i < n_weeks; i++){
            for(int j = 7*i; j < 7*(i + 1); j++){
                mgrt_vals[j] = best_vec[i];
            }
        }
        out1 = covid19(row, t_max, realize, par, beta_vals, mgrt_vals);
...
OutputParams covid19(int iter, int t_max, int n_realz, CountyParams par, double *beta_vals, double *mgrt_vals) {
    ...
    std::copy ( mgrt_vals, mgrt_vals + t_max, input_m.begin());
