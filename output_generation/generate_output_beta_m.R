...
for(i in 1:length(county)){
  ...
  model_data_df = read_csv(paste0(PATH_TO_FILES, "beta_m_group/", this_county, "_params_all_counts_5000_all_pops.csv"))
  ...
  grid_param_df =  read_csv(paste0(PATH_TO_FILES, "beta_m_group/", this_county, "_full_covid_list_5000_all_pops.csv"), 
                            col_names = FALSE)
  ...
  top_m_df = top_param_df[, c((n_weeks+1): (2*n_weeks))] %>%
...
ggsave(filename = paste0("regions", ".png"),
       plot = plot_combo_pop,
       path = paste0(PATH_TO_FILES, "beta_m_group/"),
       device = "png",
       height = 3,
       width = 8,
       units = "in",
