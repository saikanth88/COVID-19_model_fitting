...
double LHood_Poisson(int **data_hosp, int n_days, int win_count, int window_size, double *param, BestParams best, double *daily_beta)
{
...
//    cout<<day<<"\t"<<pop<<"\t"<<n_hosp[day - 1][pop - 1]<<"\n";
  ...
  n_rows = 0;
  string str_beta, line_beta;
  string beta_file = region+"_daily_beta.csv";
  fin.open(beta_file);
  fin.ignore(1000, '\n');
  while(getline(fin, line_beta))
     n_rows++;
  fin.close();

  double *daily_beta = new double[n_rows];

  int row = 0;
  fin.open(beta_file);
  fin.ignore(1000, '\n');
  while(getline( fin, line_beta)){
     istringstream iss(line_beta);
     while (getline(iss, str_beta, ',')){}
        stringstream(str_beta) >> daily_beta[row];
//        cout<<daily_beta[row]<<"\n";
        row++;
  }
  fin.close();
  ...
  param_min[0] = 0.0,   param_max[0] = 0.08; //m
...
//        param_temp[0] = best.beta_best[win_count-1];
        param_temp[0] = best.mgrt_best[win_count-1];
    }
    ...
    temp_max = LHood_Poisson(n_hosp, n_days, win_count, window_size, param_temp, best, daily_beta);
    max_lhood = temp_max;
...
//    best.beta_best[win_count] = param_best[0];
    best.mgrt_best[win_count] = param_best[0];
...
//  fout1.close();
  ...
  double full_lhood = LHood_Poisson(n_hosp, n_days, n_windows-1, window_size, NULL, best, daily_beta);
